...
public class BookAuthorApp {
	...
	public static void main(String[] args) {
		...
		// bookSelect
		List<BookVo> bookAllList = bookDao.getAllBookList();

		bookAllPrint(bookAllList);
		...
		bookAllList = bookDao.getAllBookList();
		bookAllPrint(bookAllList);
	...
	public static void searchWord(List<BookVo> bookAllList, String word) {
		for (BookVo books : bookAllList) {
			if (books.getBookTitle().contains(word) || books.getBookPubs().contains(word)
					|| books.getAuthorName().contains(word) || books.getAuthorDesc().contains(word)) {
				System.out.println(books.getBookId() + ", " + books.getBookTitle() + ", " + books.getBookPubs() + ", "
						+ books.getBookPubDate() + ", " + books.getAuthorId() + ", " + books.getAuthorName() + ", "
						+ books.getAuthorDesc());
			}
		}
	}
	
	public static void bookAllPrint(List<BookVo> bookAllList) {
		for (int i = 0; i < bookAllList.size(); i++) {
			BookVo vo = bookAllList.get(i);
			System.out.println(
					vo.getBookId() + ", " + vo.getBookTitle() + ", " + vo.getBookPubs() + ", " + vo.getBookPubDate()
